...
- [ ] T007 [P] Implement a small constitution read/write helper to read/write YAML block under `Acronyms` in `memory/constitution.md` — src/lib/constitution.ts
...
**Independent Test**: Run the configuration CLI flow and verify `memory/constitution.md` contains an `Acronyms` entry and that `templates/spec-template.md` renders `{{ACRONYM:User Story}}-1` correctly via the resolver.
...
- [ ] T021 [US3] Add `BranchConfig` and `JiraMapping` example entries to `memory/constitution.md` (update example block) — memory/constitution.md
